...
async function storeState(userId, provider) {
...
async function handleGmailCallback(code, state) {
    ...
    if (!accessToken)
        badRequest("Missing access token");
    ...
    if (!emailAddress)
        badRequest("Unable to determine Gmail account email address");
...
router.post("/link/gmail", requireUser, async (request, response, next) => {
    try {
...
router.post("/link/outlook", requireUser, async (request, response, next) => {
    try {
...
router.get("/accounts", requireUser, async (request, response, next) => {
    try {
...
router.delete("/accounts/:accountId", requireUser, async (request, response, next) => {
    try {
