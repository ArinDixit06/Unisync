...
export function isMailArmored(value) {
    return typeof value === "string" && value.trim().startsWith(MAIL_ARMOR_HEADER);
}
export function encryptMailText(value) {
    if (value == null)
        return null;
    return armor(encrypt(value.slice(0, MAX_SECURE_TEXT_CHARS)));
}
export function decryptMailText(value) {
    if (value == null)
        return null;
    if (!isMailArmored(value))
        return value;
    return decrypt(unarmor(value));
}
export function encryptMailJson(value) {
    if (value == null)
        return null;
    return encryptMailText(JSON.stringify(value));
}
export function decryptMailJson(value, fallback) {
    if (value == null)
        return fallback;
