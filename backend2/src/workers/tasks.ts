...
function fallbackEvents(bodyHtml: string, subject?: string | null): any[] | null {
...
function choosePriority(aiPriority: any | null, deterministic: any | null): any | null {
  if (!aiPriority) return deterministic;
  if (!deterministic) return aiPriority;
  const order: Record<string, number> = { low: 0, medium: 1, high: 2 };
  if (order[deterministic.priority] > order[aiPriority.priority]) return deterministic;
  if (order[aiPriority.priority] > order[deterministic.priority]) return aiPriority;
  return {
...
export async function processEmail(emailId: string): Promise<void> {
  ...
  let priority: any | null = null;
  let summary: string[] | null = null;
  let aiRisk: any | null = null;
  let events: any[] | null = null;

  try { category = await categoryClassification(email.sender_email, email.subject || "", email.preview_snippet || ""); } catch {}
  try { priority = await priorityAnalysis(email.sender_email, email.subject || "", bodyHtml); } catch {}
  try { summary = await summarizeEmail(bodyHtml); } catch {}
  if (detRiskLevel !== "high") {
    try { aiRisk = await phishingAnalysis(email.sender_email, email.subject || "", bodyHtml); } catch {}
  }
  try { events = await extractEvents(bodyHtml); } catch {}
  if (!events) events = fallbackEvents(bodyHtml, email.subject);

  const finalPriority = choosePriority(priority, detPriority);
  const riskOrder: Record<string, number> = { low: 0, medium: 1, high: 2 };
  let finalRisk = detRiskLevel;
  if (aiRisk && riskOrder[aiRisk.risk] > riskOrder[finalRisk]) finalRisk = aiRisk.risk;
