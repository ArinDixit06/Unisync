...
function domainFromEmail(address) {
...
function includesSuspiciousUnicode(value) {
    return /[\u200B-\u200F\u202A-\u202E]/.test(value);
}
function hasTrustedAcademicDomain(domain) {
...
export function sanitizeHeaders(headers) {
    ...
    for (const [key, value] of Object.entries(headers)) {
        ...
        if (allowed.has(normalized))
            sanitized[normalized] = String(value).slice(0, 500);
    }
    return sanitized;
}
export function deterministicRisk(senderEmail, senderName, headers, subject, bodyHtml) {
    let riskScore = 0;
    const reasons = [];
...
export function deterministicPriority(senderEmail, subject, bodyHtml) {
    const textBlob = `${subject ?? ""} ${bodyHtml ?? ""}`.toLowerCase();
    let priority = "low";
    const reasons = [];
    ...
    if (priority === "low" && !reasons.length)
        return null;
    return {
        priority,
