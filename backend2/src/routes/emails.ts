...
router.post("/:emailId/insights", async (request: AuthenticatedRequest, response, next) => {
  try {
    const emailId = String(request.params.emailId)
    const body = (request.body || {}) as InsightsRequestBody;
    ...
    const result = await emailInsights(
      email.sender_email || "",
      email.subject || "",
      bodyText,
      question
