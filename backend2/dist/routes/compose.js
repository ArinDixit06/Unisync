...
async function buildRawEmail(payload, fromEmail, to, cc, bcc) {
    const customHeaders = {};
    if (payload.in_reply_to)
        customHeaders["In-Reply-To"] = String(payload.in_reply_to);
    if (payload.references)
        customHeaders["References"] = String(payload.references);
...
async function handleSend(request, response, next) {
    try {
        ...
        if (!to.length && !cc.length && !bcc.length)
            badRequest("Please add at least one recipient.");
        ...
        if (!accounts.length)
            notFound("Linked account not found");
...
router.put("/drafts/:draftId", async (request, response, next) => {
    try {
