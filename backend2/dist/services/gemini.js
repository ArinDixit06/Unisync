...
function boundedText(value, limit = 6000) {
...
function extractJson(text) {
    ...
    try {
        ...
        if (startList !== -1 && endList > startList)
            return JSON.parse(candidate.slice(startList, endList + 1));
        throw new Error("Unable to parse Gemini JSON");
    }
}
async function callJson(modelName, prompt) {
    if (circuitOpen())
...
function flashModelCandidates() {
    const preferred = flashModel().trim();
    const ordered = [preferred, ...FLASH_MODEL_FALLBACKS];
    return [...new Set(ordered.filter(Boolean))];
}
async function callJsonWithFallbacks(modelNames, prompt) {
    let lastError = null;
...
export async function summarizeEmail(text) {
    ...
    if (!bounded)
        return null;
    const data = await callJson(flashModel(), `Summarize this student-facing email into exactly 3 concise bullet points. Each bullet must be 6 to 15 words. Return JSON only as {"bullets":[..]}.\n\nEMAIL:\n${bounded}`);
    const bullets = Array.isArray(data?.bullets) ? data.bullets.map((item) => String(item).trim().slice(0, 120)).filter(Boolean) : [];
    return bullets.length === 3 ? bullets : null;
}
export async function priorityAnalysis(sender, subject, body) {
    const data = await callJson(flashModel(), `Assign priority for a student inbox. Consider deadlines, account actions, schedule changes, exams, finances, and urgency. Return JSON only as {priority:'high'|'medium'|'low',reason:string,confidence:float}.\n\nSENDER:${boundedText(sender, 200)}\nSUBJECT:${boundedText(subject, 300)}\nBODY:${boundedText(body)}`);
...
export async function categoryClassification(sender, subject, snippet) {
    const data = await callJson(flashModel(), `Classify a student email into one of: primary, updates, promotions, social, forums. Return JSON only as {category:'primary'|'updates'|'promotions'|'social'|'forums'}.\n\nSENDER:${boundedText(sender, 200)}\nSUBJECT:${boundedText(subject, 300)}\nSNIPPET:${boundedText(snippet, 600)}`);
    const category = String(data?.category ?? "").toLowerCase();
    return ALLOWED_CATEGORIES.has(category) ? category : null;
}
export async function phishingAnalysis(sender, subject, body) {
    const data = await callJson("gemini-1.5-pro", `Analyze this email for phishing, impersonation, credential theft, payment fraud, or manipulative urgency. Return JSON only as {risk:'low'|'medium'|'high',reasons:[string]}.\n\nSENDER:${boundedText(sender, 200)}\nSUBJECT:${boundedText(subject, 300)}\nBODY:${boundedText(body)}`);
    const risk = String(data?.risk ?? "").toLowerCase();
    if (!ALLOWED_RISKS.has(risk))
        return null;
    return {
        risk,
        reasons: Array.isArray(data?.reasons) ? data.reasons.map((item) => String(item).trim().slice(0, 160)).filter(Boolean).slice(0, 5) : []
    };
}
export async function extractEvents(body) {
    const bounded = boundedText(body);
    if (!bounded)
        return null;
    const data = await callJson("gemini-1.5-pro", `Extract real events or deadlines from this email body. Return JSON list only. Each item must have title, date, time, location, description, confidence. Only include items with confidence greater than 0.8.\n\nBODY:\n${bounded}`);
...
export async function emailInsights(sender, subject, body, question) {
    ...
    const prompt = "You are an AI assistant helping a university student understand their email.\n" +
    ...
    try {
        ...
        const answer = String(data.answer ?? "").trim().slice(0, 800) || "No answer generated.";
        const keyPoints = Array.isArray(data.key_points)
            ? data.key_points
                .map((point) => String(point).trim().slice(0, 200))
                .filter(Boolean)
                .slice(0, 5)
            : [];
        const suggestedAction = String(data.suggested_action ?? "").trim().slice(0, 300) || null;
        return {
            answer,
