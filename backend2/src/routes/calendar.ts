...
router.post("/events/:eventId/confirm", async (request: AuthenticatedRequest, response, next) => {
  try {
    ...
    if (!event.start_datetime) badRequest("Event date missing");
    ...
    if (!accounts.length) badRequest("No Gmail account linked for calendar sync");
    ...
    try {
      ...
      if (!accessToken) badRequest("Unable to refresh Gmail token for calendar access");
      await update(
        "linked_accounts",
        { access_token_enc: encrypt(accessToken) },
        { filters: [["id", "eq", account.id]], userToken: request.currentUser!.token }
      );
      result = await createEvent(accessToken, event.title, event.start_datetime, event.end_datetime, event.location, event.description);
    }
    await update(
